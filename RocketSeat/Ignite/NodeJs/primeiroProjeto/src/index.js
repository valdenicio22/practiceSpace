...
function verifyIfExistsAccountCPF(request, response, next) {
  ...
  if (!customer) return response.status(400).json({ error: "Costumer not found" })
...
app.post("/account", (request, response) => {
  ...
  const customerAlreadyExists = customers.some(costomer => costomer.cpf === cpf);
  if (customerAlreadyExists) response.status(400).json({ Error: "CPF already Exist!" });
